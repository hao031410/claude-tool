...
## 1. 文件路径
- `${LOCAL_DB_DIR:-~/data}/terminus-emp-skill/state.json`
...
## 4. 写入要求
1. 原子写：`state.json.tmp -> rename -> state.json`。
2. 文件权限：`600`。
3. 写入失败时不得覆盖旧文件。
...
4. `stats_cache.daily[yyyy-MM-dd]`：
   - `status`
   - `details`
   - `ratio`
   - `lastSuccessAt`
   - `sourceRequestId`
5. `stats_cache.weekly[yyyy-WW]`：
   - `workdayCount`
   - `filledDays`
   - `fillRate`
   - `missing`
6. `stats_cache.monthly[yyyy-MM]`：
   - `workdayCount`
   - `filledDays`
   - `fillRate`
   - `summaryByProject`
